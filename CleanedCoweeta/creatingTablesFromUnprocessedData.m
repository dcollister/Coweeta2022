function [PreResultsStructure]=creatingTablesFromUnprocessedData(NamesandParamsTable)

    if ~isfile('SimResults\CoweetaCollectedNeedProcessing.mat')
%% Create Structure
...
%% Left off here.  Structure built with OFAT arrays and info included.      
%     TempPBTable=TempPlacementTable(OFATic==OFATCount,:);
%     PBNamesArray = table2cell(TempPBTable(:,5));
%     UnCollectionArray=repmat({'Combined'},height(PBNamesArray),1);
%     PBNamesArrayIndices = ~cellfun(@contains,PBNamesArray,UnCollectionArray);
%     UniquePBValsTable = TempPBTable(PBNamesArrayIndices,:);
%     UniquePBValsArray = table2cell(UniquePBValsTable(:,9));
%     [ProbReproductionVals,PBValsIa,PBValsIc] = unique(UniquePBValsTable.ProbReproduction);
% 
%     TempOFATtable=UniquePBValsTable(PBValsIa,6:9);
%     TempOFATtable.Properties.RowNames=UniquePBValsTable(PBValsIa,:).PBName;
% 
%     for OFATCount=1:height(OFATNames)
%     UnCollectionArray=repmat({'Combined'},height(PBNamesArray),1);
%     PBNamesArrayIndices = ~cellfun(@contains,PBNamesArray,UnCollectionArray);
...
%     CollectionsArray=repmat({'ProbBirth_'},height(NamesandParamsTable),1);
    ...
    if ~(isfile([fileShell 'CollectedFileNamesAndParamValuePieces.mat']))
        for tableCount=1:height(C)
            ...
            for fileGrabCount=1:sum(ic==tableCount)
                fileGrabName=strcat(tableFiles{fileGrabCount,1},'\',tableFiles{fileGrabCount,2},'\',tableFiles{fileGrabCount,3},'\*.mat');
...
%                     UAFileNames(runCount)=this_folder;
        ...
        load('SimResults\CoweetaCollectedNeedProcessing.mat','PreResultsStructure')
    end
