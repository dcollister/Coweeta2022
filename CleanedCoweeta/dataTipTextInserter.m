function output_txt = dataTipTextInserter(obj,event_obj)
    ...
    pos = obj.Position;%event_obj.Position;


    %********* Define the content of the data tip here *********%

    % Display the x and y values:
    output_txt = {['Lattitude',formatValue(pos(1),event_obj)],...
        ['Longitude',formatValue(pos(2),event_obj)]};
    %***********************************************************%


    % If there is a z value, display it:
    if length(pos) > 2
        output_txt{end+1} = ['Agent Count',formatValue(pos(3),event_obj)];
    end
