function [CoweetaCollectedNeedProcessing]=combiningTablesFromOFATSingleData(CoweetaCollectedNeedProcessing,singleOFATFilename)
    
    singlefileParamName=strcat(singleOFATFilename, '\NamesandParamsTable.mat');
    ...
    WeirdCount=0;
    for fileCount=1:height(CoweetaCollectedNeedProcessing)
    if strcmp(CoweetaCollectedNeedProcessing.ProcessedName(fileCount),"")
        wholeFolderName=strcat(CoweetaCollectedNeedProcessing.PBName(fileCount),'\');
        PBName=CoweetaCollectedNeedProcessing.PBName{fileCount};
        %         combinedFilename=strcat(CoweetaCollectedNeedProcessing.OFATName{fileCount},'\CombinedProbBirth_',num2str(CoweetaCollectedNeedProcessing.ProbReproduction{fileCount}),'\');
        for tableNameCount=1:4
            project_dir = wholeFolderName;%tables{tableCount}.UAFolderName{tableFileCount};%['SimResults\',RunSaveFolder,runName];%pwd();     %or give a particular directory name
            TableType=TablesNames{tableNameCount};

            mainDirContents = dir(strcat(wholeFolderName,TablesNames{tableNameCount},'*.csv'));
            ...
            for runCount=1:num_subfolder
                ...
                data=readtable(strcat(wholeFolderName,this_folder));
                if width(data)<30 && width(data)>22
                    ...
                    data.RunID = cellstr(data.RunID);
                    data.RunID = categorical(data.RunID);
                    writetable(data,strcat(wholeFolderName,this_folder))
                end

                if ~isempty(data) && width(data)>22
                    k=k+1;
                    data.RunID = categorical(data.RunID);
                    data.AgentType=categorical(data.AgentType);
                    if runCount==1
                        compTable= table(data.RunID,data.RadialMaxCoords,data.RadialMaxTime,data.RMaxCoords_1,data.RMaxCoords_2,...
                            ...
                            data.deathdiffX,data.DeathTime,data.AgentType,...
                            'VariableNames',{'RunID','RadialMax','RadialMaxTime','RMaxCoords_1','RMaxCoords_2',...
                            'BirthCoords_1','BirthCoords_2','DeathCoords_1','DeathCoords_2','deathdiffX','DeathTime','Death Type'});
                    else
            ...
            if ~isempty(compTable)
                ...
                UASaveFolder=strcat(PlacementSaveHold,'CombinedFiles',UAhold);
                UAFigFile=strcat(UASaveFolder,'Figures\');
                OFATfolder=strcat(PlacementSaveHold,'CollectedRunsMeans\',AgentType,'\');
                


            if isempty(CoweetaCollectedNeedProcessing.ProcessedName{fileCount})
                CoweetaCollectedNeedProcessing.ProcessedName(fileCount)=UASaveFolder;
            end
                ...
                if ~isfile(strcat(UASaveFolder,'\',TableType,'.mat'))
                    ...
                    if stats2.df>0 && size(stats2.gnames,1)>1
                        [c,m,h,nms] = multcompare(stats2);
                        [nms num2cell(m)];
                    end
                    saveas(hTable,strcat(UAFigFile, AgentType, 'Table.fig'))
                    saveas(hBoxPlot,strcat(UAFigFile, AgentType, 'BoxPlots.fig'))
                    saveas(MeansComp,strcat(UAFigFile, AgentType, 'MeanComp.fig'))
                    close(hTable);
                    close(hBoxPlot);
                    close(MeansComp);
                    ...
                    if p>0.95 
                        ...
                        VariationFiles{WeirdCount}=strcat(UASaveFolder,'\',TableType,'.csv');
                    end
                    ...
                    writetable(compTable,strcat(UASaveFolder,'\',TableType,'.csv'));
                    save(strcat(UASaveFolder,'\',TableType,'.mat'),'compTable')
                end

            else
            end
        end
    NamesandParamsTable=CoweetaCollectedNeedProcessing;
    save(singlefileParamName,'NamesandParamsTable');

    else
    load(singlefileParamName,'NamesandParamsTable');
    CoweetaCollectedNeedProcessing=NamesandParamsTable;
    end
    end

    if WeirdCount>0
        WeirdSaves=strcat(singleOFATFilename,'\OutlierRuns.mat');
        save(WeirdSaves,'VariationFiles','-v7.3')
    end
end


%             mainDirContents = dir(wholeSearchFolderName);
%             mask = ismember( {mainDirContents.name}, {'.', '..'});
%             mainDirContents(mask) = [];
%             namesArray={mainDirContents.name}.';
%
%             %%All Agents
%             AllCollectionArray=repmat({'AllCollected'},height(namesArray),1);
%             AllAgsID=cellfun(@contains,namesArray,AllCollectionArray);
%             Allnames=namesArray(AllAgsID);
%             for idCount=1:sum(AllAgsID)
%                 UACompareAllTables{idCount}=readtable(strcat(wholeFolderName,Allnames{idCount}));
%             end
%             AllTable=vertcat(UACompareAllTables{:});
%
%             %%Repro Agents
%             ReproCollectionArray=repmat({'ReproCollected'},height(namesArray),1);
%             ReproAgsID=cellfun(@contains,namesArray,ReproCollectionArray);
%             Repronames=namesArray(ReproAgsID);
%             if ~isempty(Repronames)
%                 for idCount=1:sum(ReproAgsID)
%                     UACompareReproTables{idCount}=readtable(strcat(wholeFolderName,Repronames{idCount}));
%                 end
%                 ReproTable=vertcat(UACompareReproTables{:});
%             end
%
%             %%Boundary Deaths
%             BoundCollectionArray=repmat({'BoundCollected'},height(namesArray),1);
%             BoundAgsID=cellfun(@contains,namesArray,BoundCollectionArray);
%             Boundnames=namesArray(BoundAgsID);
%             if ~isempty(Boundnames)
%                 for idCount=1:sum(BoundAgsID)
%                     UACompareBoundTables{idCount}=readtable(strcat(wholeFolderName,Boundnames{idCount}));
%                 end
%                 BoundTable=vertcat(UACompareBoundTables{:});
%             end
%
%             NonRNonBCollectionArray=repmat({'NonRNonBCollected'},height(namesArray),1);
%             NonRNonBAgsID=cellfun(@contains,namesArray,NonRNonBCollectionArray);
%             NonRNonBnames=namesArray(NonRNonBAgsID);
%             if ~isempty(NonRNonBnames)
%                 for idCount=1:sum(NonRNonBAgsID)
%                     UACompareNonRNonBTables{idCount}=readtable(strcat(wholeFolderName,NonRNonBnames{idCount}));
%                 end
%                 NonRNonBTable=vertcat(UACompareNonRNonBTables{:});
%             end
%
%             %%Create and save files
%             if ~isfile(combinedFilename)
%         end
%
