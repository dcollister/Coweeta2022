 function plotTopFlyersAgainstGradient(ElevationGrid,RiverCoords,A,landscapeName,GraphTitle,graphFilename,singleGraphFilename,imageStorLoc,ofatParamString,...
        plotType,singleX,singleY,sourceRadius,innerCatchRadius,outerCatchRadius,parameterValues)

if ~isfile([landscapeName '.fig'])
LandScapeBaseFig=figure('WindowState','maximized');
    axesLandScapeFig = axes('Parent',LandScapeBaseFig);
    hold(axesLandScapeFig,'on');
[c1,h1] = contourf(axesLandScapeFig,X,Y,Z,'TextStep',100,'LineWidth',2,'ZLocation','zmin');
    clabel(c1,h1); 
       
    colormap('jet');
    set(gca,'visible','off')
    set(axesLandScapeFig,'Colormap',...
    ...
    % Create colorbar
    colorbar(axesLandScapeFig,'FontSize',12);
    h=colorbar;
    set(get(h,'label'),'string','Elevation in feet');
scatter(RiverCoords(:,1),RiverCoords(:,2),47,'filled','hexagram','MarkerFaceColor','black','Parent',axesLandScapeFig);
scatter(RiverCoords(:,1),RiverCoords(:,2),45,'filled','hexagram','MarkerFaceColor','white','Parent',axesLandScapeFig);
% scatter(TopFlyers.BirthCoords_1,TopFlyers.BirthCoords_2,28,'black','o','filled');
% scatter(TopFlyers.DeathCoords_1,TopFlyers.DeathCoords_2,14,'yellow','o','filled');
% scatter(TopFlyers.BirthCoords_1,TopFlyers.BirthCoords_2,22,'#183EFA','filled','o');
% quiver(axesLandScapeFig,X,Y,DX,DY);
    hold(axesLandScapeFig,'off');
    savefig(LandScapeBaseFig,[landscapeName '.fig'],'compact');
    saveas(LandScapeBaseFig,[landscapeName '.jpg']);
    close(LandScapeBaseFig);
else 
%     openfig([landscapeName '.fig'])
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%  Plot ags against structure using multi-dim script %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


singleLocationAnalysis(landscapeName,imageStorLoc,A,plotType,singleX,singleY,sourceRadius,innerCatchRadius,outerCatchRadius,parameterValues)
Z=ElevationGrid;                
[X,Y]=meshgrid(1:size(ElevationGrid,2),1:size(ElevationGrid,1));
[DX,DY]=gradient(ElevationGrid,10);
%% Designates the subpopulation to use
reproMat=ismember(A.DeathType,'Reproducing Agent');
ReproAgs=A(reproMat,:);
boundMat=ismember(A.DeathType,'Boundary Escape');
boundAgs=A(boundMat,:);
DeadzosMat=ismember(A.DeathType,'Death From Environment');
DeadzosAgs=A(DeadzosMat,:);
distFromSing=sqrt((A.BirthCoords_1-384).^2+(A.BirthCoords_2-404).^2);
singAgs=A(distFromSing<10,:);
radMaxValsQuants = quantile(A.RadialMax,[.025 0.05 .25 .50 .75 0.95 .975]);
DeathDistValsQuants = quantile(A.deathdiffX,[.025 0.05 0.1 .25 .50 .75 0.90 0.95 .975]);
radMaxReproValsQuants = quantile(ReproAgs.RadialMax,[.025 0.05 .25 .50 .75 0.95 .975]);
DeathDistReproValsQuants = quantile(ReproAgs.deathdiffX,[.025 0.05 0.1 .25 .50 .75 0.90 0.95 .975]);
[vecReproTopFlyers,idTopFlyers,FlipIDTopFlyers]=find(ReproAgs.RadialMax>radMaxReproValsQuants(7));
[vecReproTopDeaths,idTopDeaths,FlipIDTopDeaths]=find(ReproAgs.deathdiffX>DeathDistReproValsQuants(7));
[vecReproHomeBodies,idHomes,FlipIDHomes]=find(ReproAgs.deathdiffX<25);
ReproTopFlyers=ReproAgs(vecReproTopFlyers,:);
ReproTopDeaths=ReproAgs(vecReproTopDeaths,:);
ReproHomeBodies=ReproAgs(vecReproHomeBodies,:);
ReproTabs={ReproAgs,ReproTopFlyers,ReproTopDeaths,ReproHomeBodies,boundAgs,DeadzosAgs};
subtitles={'All Repro Agents', '95% or Greater Repro Flyers from home', '95% or Greater distance Breeders', 'Close to home breeders (25 feet or less', 'Boundary Counts and Origins','Traveling isnt for the faint of heart agents'};
subtitlesForSaves={'All_Repro_Agents', '95%_or_Greater_Repro_Flyers_from_home', '95%_or_Greater_distance_Breeders', 'Close_to_home_breeders_(25_feet_or_less', 'Boundary_Counts_and_Origins','Traveling_isnt_for_the_faint_of_heart_agents'};
% MultiDimHistoSingGenSubPopSpecificFromTable(A,ReproTabs,GraphTitle,subtitles,subtitlesForSaves,graphFilename,singleGraphFilename,ofatParamString);
% singleLocationAnalysis(parameterValues);
end
