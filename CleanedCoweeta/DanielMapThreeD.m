function resultsStructure=DanielMapThreeD(agentsInfoTable,Elevation,River,varType,varCoords,sheetCount,tilecount,resultsStructure,resultsCount)
%% 2D information found here
...
PopDistHist=histogram2(testAgTable{:,varCoords{1}},testAgTable{:,varCoords{2}},[binsYRows binsXCols],'DisplayStyle','bar3','EdgeAlpha',0.5,'FaceColor','flat');
...
rivScat=scatter(rivColX,rivRowY,12,'MarkerEdgeColor','k','MarkerFaceColor','k','MarkerFaceAlpha',1);
...
w = PopC.LineWidth;
PopC.LineWidth = 1.5;
cellSizeString=strcat(num2str(intervalWidth),'x', num2str(intervalWidth), ' m cell');
title(PopC,{'# Agents', cellSizeString});
Names=strcat(string(PopC.Ticks),' or ~ ',string(PercentLabels(1:width(PopC.Ticks))),'%');
...
titleName={'Coweeta River Landscape'};
title(titleName)
