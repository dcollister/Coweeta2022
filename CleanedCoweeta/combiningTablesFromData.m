function combiningTablesFromData(CoweetaCollectedNeedProcessing)
    ...
    WeirdCount=1;
    for fileCount=1:width(CoweetaCollectedNeedProcessing)
        outerfileGrabName=strcat(CoweetaCollectedNeedProcessing(fileCount).AgentPlacement,'\',CoweetaCollectedNeedProcessing(fileCount).OFATNames);
        for OFATCount=1:height(CoweetaCollectedNeedProcessing(fileCount).PBNameArray)
            wholeSearchFolderName=strcat(outerfileGrabName,CoweetaCollectedNeedProcessing(fileCount).PBNameArray(OFATCount),'\*.csv');
            wholeFolderName=strcat(outerfileGrabName,CoweetaCollectedNeedProcessing(fileCount).PBNameArray(OFATCount),'\');
            PBName=CoweetaCollectedNeedProcessing(fileCount).PBNameArray{OFATCount};
            combinedFilename=strcat(outerfileGrabName,'\Combined',PBName(2:end),'\');
            for tableNameCount=1:4
                ...
                for runCount=1:num_subfolder
                    k=0;
                    this_folder = fullfile( mainDirContents(runCount).name );%readtable(strcat(wholeFolderName,Allnames{idCount})
    ...
    if ~isempty(WeirdCount)
        save('SimResults\OutlierRuns.mat','VariationFiles','-v7.3')
    end
